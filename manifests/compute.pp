...
class openstack::compute (
  ...
  if ! $quantum {
    ...
    $quantum_sql_connection = "mysql://${quantum_db_user}:${quantum_db_password}@${db_host}/${quantum_db_name}?charset=utf8"

    if ! $quantum_db_password {
      fail('quantum sql password must be specified when quantum is installed on compute instances')
    }
